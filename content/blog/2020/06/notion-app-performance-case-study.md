...
번들 초기화는 굉장히 많은 시간을 할애하는데, 그 이유는 모든 모듈을 실행하기 떄문이다. 각 모듈은 실행하는데 몇 밀리초가 걸릴뿐이지만, 노션의 경우 이러한 모듈이 1100개가 넘게 있다. 이것을 해결하는 유일한 방법은 초기화에 더 적은 모듈을 실행하는 것이다.
...
보통 앱이 뭔가 로딩 중인 것을 보여주고 싶을 떄 스피너나 로딩 바를 쓰곤 한다. [그러나 때로는 스피너가 체감상 성능을 더 악화시키는 효과를 가질 때가 있다.](https://www.lukew.com/ff/entry.asp?1797) 유저는 스피너를 보고 앱이 더 느리다고 느낄 수 있다. 이러한 느낌을 피하기 위해서는, 스켈레톤 UI를 사용하면 좋다.
